  ...
  switch (item.controlName) {
    ...
    case FormControlNames.CHECKBOX:
      return (
        <>
          ...
          <FormControl>
            <RadioGroup name={item.controlName + item.internalId}>
              {item.items?.map((i) => (
                <FormControlLabel
                  value={i.value}
                  key={i.value}
    ...
    case FormControlNames.MULTICHOICES:
      return (
        <>
          <FormControl>
            <RadioGroup name={item.controlName + item.internalId} row>
              {item.items?.map((i) => (
                <FormControlLabel
                  value={i.value}
                  key={i.value}
    ...
    case FormControlNames.FILEUPLOAD:
      return (
        <>
          <input
            style={{ display: "none" }}
            id={item.controlName + item.internalId}
            type="file"
          />
          <label
            className="control-input-trigger-buttons"
            htmlFor={item.controlName + item.internalId}
          >
...
function ControlViewComponent(props: ControlViewComponentProps) {
  ...
  if (
    selectedControl &&
    item.internalId === selectedControl.internalId &&
  ...
  const handleDeleteControl: React.MouseEventHandler<HTMLSpanElement> = (
    ...
    deleteControl(item.internalId, containerId);
